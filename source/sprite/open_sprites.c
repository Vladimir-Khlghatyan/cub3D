...
{
	c->xpm[16].ptr = mlx_xpm_file_to_image(c->mlx, SPRT_PIC_ME, \
		&c->xpm[16].width, &c->xpm[16].height);
	c->xpm[16].addr = mlx_get_data_addr(c->xpm[16].ptr, &c->xpm[16].bpp, \
									&c->xpm[16].linelen, &c->xpm[16].endian);
	c->xpm[17].ptr = mlx_xpm_file_to_image(c->mlx, DOOR_PIC1, \
		&c->xpm[17].width, &c->xpm[17].height);
	c->xpm[17].addr = mlx_get_data_addr(c->xpm[17].ptr, &c->xpm[17].bpp, \
									&c->xpm[17].linelen, &c->xpm[17].endian);
	c->xpm[18].ptr = mlx_xpm_file_to_image(c->mlx, DOOR_PIC2, \
		&c->xpm[18].width, &c->xpm[18].height);
	c->xpm[18].addr = mlx_get_data_addr(c->xpm[18].ptr, &c->xpm[18].bpp, \
									&c->xpm[18].linelen, &c->xpm[18].endian);
	c->xpm[19].ptr = mlx_xpm_file_to_image(c->mlx, DOOR_PIC3, \
		&c->xpm[19].width, &c->xpm[19].height);
	c->xpm[19].addr = mlx_get_data_addr(c->xpm[19].ptr, &c->xpm[19].bpp, \
									&c->xpm[19].linelen, &c->xpm[19].endian);
	c->xpm[20].ptr = mlx_xpm_file_to_image(c->mlx, DOOR_PIC4, \
		&c->xpm[20].width, &c->xpm[20].height);
	c->xpm[20].addr = mlx_get_data_addr(c->xpm[20].ptr, &c->xpm[20].bpp, \
									&c->xpm[20].linelen, &c->xpm[20].endian);
}
